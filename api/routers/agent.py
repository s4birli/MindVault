...
def _detect_intent_and_params(text: str, user_params: Optional[Dict[str, Any]] = None) -> Tuple[Optional[str], Optional[Dict[str, Any]]]:
    """
    ...
    Returns:
        Tuple of (intent, merged_params) or (None, None) if no match
    """
    text_lower = text.lower()
    
    # Rule 1: HMRC email search (specific case)
    if "hmrc" in text_lower and ("email" in text_lower or "emailleri" in text_lower):
    ...
    # Rule 2: General email search with sender extraction
    # Look for patterns like "X'dan gelen email" or "emails from X" or "X emailleri"
    ...
    sender_patterns = [
        ...
        r"(\w+)\s+emailleri",  # Turkish: "Michael emailleri"
        r"(\w+)\s+mailler",    # Turkish: "Michael mailler"
    ]
    ...
    if any(keyword in text_lower for keyword in email_keywords):
        ...
        if sender:
            ...
            # Try to extract limit from text (e.g., "son 2 mailler", "last 3 emails")
            limit = 5  # default
            limit_patterns = [
            ...
            for limit_pattern in limit_patterns:
                ...
                if limit_match:
                    limit = min(50, max(1, int(limit_match.group(1))))  # Cap between 1-50
                    break
